...
const sheetWiseTotalProcessedRowsCount = {};
  ...
  try {
    ...
    await processFileLines(filePath, async (line, lineNumber) => {
      ...
      try {
        ...
        sheetWiseTotalProcessedRowsCount[currentSheetName] = sheetWiseTotalProcessedRowsCount[currentSheetName] || 0;
        sheetWiseTotalProcessedRowsCount[currentSheetName]++;
        if (header.length === 0) {
        ...
        if (sheetWiseTotalProcessedRowsCount[currentSheetName] === 1) {
          await callBack({
            event: 'create',
            columns: header
              .filter(c => !c.skip)
